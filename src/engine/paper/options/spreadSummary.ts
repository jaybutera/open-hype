...
/**
 * Simplistic strategy classifier. Real strategy detection (Iron Condor, Iron
 * Butterfly, Calendar, Diagonal, etc.) is iteration 19; this is a placeholder
 * that covers the obvious single- and two-leg cases and falls back to a leg
 * count for anything more complex.
 */
export function detectSimpleStrategy(legs: OptionPosition[]): string {
  ...
  if (legs.length === 2) {
    ...
    const opposingSides = a.szi.isPositive() !== b.szi.isPositive();

    if (sameExp && sameType && !sameStrike && opposingSides) {
      return a.type === 'call' ? 'Call Vertical' : 'Put Vertical';
    }
    if (sameExp && !sameType && sameStrike && !opposingSides) {
      return a.szi.isPositive() ? 'Long Straddle' : 'Short Straddle';
    }
    if (sameExp && !sameType && !sameStrike && !opposingSides) {
      return a.szi.isPositive() ? 'Long Strangle' : 'Short Strangle';
    }
    if (!sameExp && sameType && sameStrike && opposingSides) {
      return 'Calendar';
    }
