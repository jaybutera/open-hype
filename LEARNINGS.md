...
## Iteration: 2026-04-17 14:45

### Picked
Bonus polish #2 on the remaining list: **analytical breakevens in NetSummary**. The spec's OrderForm section explicitly calls for "Breakevens (computed from leg intrinsic values)" as a footer item but the feature was never shipped — NetSummary had debit/credit and Net Greeks but no breakeven numbers. The PayoffDiagram surfaces breakeven labels visually via sampled `findBreakevens`, but there was no numeric readout before submit, and the sampled resolution (121 points over ±30%) is ~$1 on a $400 underlying. Picked over RTL/jsdom setup (infra-heavy, multi-iteration) and the strategy-aware zone visuals (pure polish on an already-functioning feature) because it closes a spec gap with a clean pure-logic + small UI change.

### Did
- `src/services/options/payoff.ts` — new `analyticalBreakevens(legs, qtyScalar?)` function. Exploits the piecewise-linear structure of expiration P&L: kinks occur only at strikes, so within each segment between adjacent strikes (plus `S=0` and a `2·maxStrike+1` probe) the function is strictly linear and zero-crossings are exact via one linear interpolation — no sampling compromise. Deduplicates crossings that coincide at segment boundaries (within `1e-9`). Segments where the payoff is identically zero (e.g. long+short of the same contract) are ignored — only sign changes are reported.
- `src/services/options/__tests__/payoff.test.ts` — 11 new tests: empty legs → []; long/short call BE at K+premium; long put at K−premium; long straddle symmetric K±(put+call premiums); call vertical debit → lower strike + net debit; iron condor → short put − credit + short call + credit (two BEs); order-independence; identically-flat payoff → []; precision check vs. 121-sample `buildPayoffCurve` (analytical within 1e-6, sampled within $2); qty scaling doesn't move BE prices.
- `src/components/options/NetSummary.tsx` — imported `analyticalBreakevens`; memoized per legs. Added a new "Breakeven" row in the per-contract/per-share grid. Format by count: `none` (gray #5a5f68) / `$X.XX` / `$X.XX – $Y.YY` / `N points` (with tooltip listing all). Only uses an em-dash between two breakevens to avoid confusion with a negative on 1-BE strategies.
- `npx tsc --noEmit` clean.
- `npm test` → 497/497 green (was 486; +11 new in `payoff.test.ts`).

### Discovered
- **`analyticalBreakevens` and `findBreakevens` are complementary, not redundant.** `findBreakevens` operates on any `PayoffSample[]` (including the "today" curve, which isn't piecewise-linear because BS is smooth). `analyticalBreakevens` only works for *expiration* P&L where the piecewise-linear shortcut applies. Kept both exported; NetSummary uses the analytical one, PayoffDiagram still uses the sampled one because its `breakevens` field comes from `buildPayoffCurve` which samples both curves for rendering anyway.
- **`crossing` helper was already in place** from `findBreakevens` iteration — private to the module. Reused directly from `analyticalBreakevens`, which meant no new sign/zero-handling logic had to be written. Single source of truth for "what counts as a sign change."
- **Segment dedup at `1e-9` tolerance** matters for identically-zero segments bounded by non-zero segments on either side: when the segment-endpoint is exactly zero, `crossing` returns the endpoint, and the next segment also evaluates to the same endpoint (shared pivot). Without the `1e-9` dedup, a short-call-at-strike-exactly-equals-premium config would report the same breakeven twice.
- **Format decision — em-dash vs. comma between 2 breakevens.** Two-BE strategies (straddle, iron condor) show `$X – $Y`. Considered `$X, $Y` but an em-dash reads "range between" which is semantically richer — between those prices the strategy is in profit (for long straddle) or loss (for short straddle). The color doesn't disambiguate because NetSummary's Breakeven row is always neutral (#e1e4e8) — it's not debit/credit colored. The 3+ BE case (rare — maybe a butterfly ratio) collapses to `N points` with a `title` tooltip listing all to avoid cramping the grid.
- **`qtyScalar` argument is accepted but doesn't affect the result.** Breakeven prices are scale-invariant — multiplying the P&L by any positive scalar doesn't move its zero-crossings. Kept the parameter for API symmetry with `expirationPnl` / `buildPayoffCurve` / `expirationExtrema`, so the function signature reads consistently and a future `useMemo([legs, qtyScalar])` caller doesn't have to special-case. Tested explicitly (qty=3 case verifies the breakeven is unchanged).
- **Integer strike math is exact in IEEE-754** — all the `toBeCloseTo(…, 9)` assertions pass at 9-decimal precision because strikes come as exact integers from the fixture and premiums are hand-picked in the tests. The epsilon tolerance in the helper is for production cases where premiums come from `legMark` (mid of bid/ask), which can have floating drift. The test precision (1e-9) exercises the analytical path's accuracy ceiling; the dedup epsilon (1e-9) is the other side of that same coin.
- **`findBreakevens` vs `analyticalBreakevens` precision gap**: analytical is within 1e-6 of the true value, sampled is within ~$1. The precision-gap test is explicit to prevent a future refactor from accidentally making the analytical function sampled.

### Next
Bonus polish list remaining:
1. **RTL/jsdom setup** — unchanged. Multi-iteration infrastructure. Unblocks component-level tests for NetSummary breakeven formatting, cap banner, cell-click interactions.
...
## Iteration: 2026-04-17 14:52

### Picked
Bonus polish #2 from the remaining list: **strategy-aware payoff zones**. Shade the contiguous underlying-price range where max profit (or max loss) is achieved — e.g. the flat band between an iron condor's short strikes, or the region above a bull call spread's upper strike. Makes the "defined reward" of a limited-risk strategy visible at a glance rather than just showing the peak point. Picked over RTL/jsdom setup (still multi-iteration infra) and the `## DONE` marker because it's a pure-logic + small SVG change that directly answers the LEARNINGS question "should the payoff module expose the flat-band's bounds?" with yes — extending `expirationExtrema`'s return shape is cleaner than having `PayoffDiagram` scan samples for runs of equal max-value.

### Did
- `src/services/options/payoff.ts` — extended `PayoffExtrema`. Each side now carries a `zone: { min, max }` field representing the contiguous underlying-price range over which the extremum holds:
  - Evaluate payoff at all pivots `{0, strikes..., 2·maxStrike+1}` into a `values[]` array (already done — now persisted for a second pass).
  - `widestRunAtValue(values, target)` finds the widest contiguous run of pivot indices whose value equals `target` within `1e-6`. "Widest" rather than "first" matters for pathological baskets with disjoint runs at the same extremum value (rare but possible with opposing ratio spreads).
  - Bounded side → zone spans the run in sorted-pivot space; unbounded side → zone collapses to `{atPrice, atPrice}` (no meaningful flat band when the payoff keeps improving past the anchor).
  - Type surface: introduced a named `PayoffExtremum` interface to DRY the shared fields across `maxProfit`/`maxLoss` rather than duplicating the shape inline.
- `src/services/options/__tests__/payoff.test.ts` — 7 new tests: empty-legs zone collapses at 0; long-call loss zone spans `[0, strike]` (payoff flat at -premium while OTM — *not* a V-shape point as I first guessed); long-put loss zone spans `[strike, far-right-probe]` (mirror); call-vertical profit zone extends from upper strike rightward, loss zone from 0 through lower strike; iron condor profit zone exactly `[shortPut.strike, shortCall.strike]`; long straddle loss zone single-point at strike; short strangle profit zone `[shortPut.strike, shortCall.strike]`.
- `src/components/options/PayoffDiagram.tsx` — rendered the zones as two additional `<rect>` fills (opacity 0.08, green for profit / red for loss), clipped to `[xScale(max(xMin, zone.min)), xScale(min(xMax, zone.max))]` and gated on `bounded && value != 0 && zone.max > zone.min` so single-point zones (V-shape extrema) don't render a zero-width rect. Layered *above* the existing triangular `aboveZero`/`belowZero` fills so the zone band reads as a deeper tint of the same color inside the P&L polygon.
- `npx tsc --noEmit` clean.
- `npm test` → 504/504 green (was 497; +7 new in `payoff.test.ts`).

### Discovered
- **Long-call max-loss is flat across `[0, strike]`, not a V-point at the strike.** My first test expected the zone to be a single point at the strike (V-shape thinking, from visualizing payoff as a hockey stick). It's wrong: for S ∈ [0, K] the call is OTM and the payoff is identically `-premium × 100`. The V-shape intuition only applies to strategies where *both* OTM and ITM sides are linear-away-from-strike (e.g. long straddle). For a plain long call, there's an entire flat floor from S=0 to S=K. Same thing mirrored for a long put: the loss band runs from K out to the right probe. The test assertions were fixed to reflect the true shape, which is now *more* informative than the naive "min/max at a point" version.
- **Widest-run vs. first-run matters in principle, not in current fixtures.** Iron condors and verticals produce exactly one contiguous extremum run, so "first" and "widest" coincide. But a leg basket like `+1 C(K1) - 2 C(K2) + 1 C(K3)` (long butterfly) has the max-profit peak *only* at K2 (single point — fine), and a short butterfly has the max-loss trough at K2 (also single point). There's no current strategy in our classifier that produces disjoint runs at the same extremum value; "widest" is a cheap safety net for pathological combos. Epsilon chosen at `1e-6` (not `1e-9` like the breakeven dedup) because premium values flow through `legMark` (mid of bid/ask) which can carry 2-3 decimal places of IEEE drift — tighter than `1e-9` but looser than `Number.EPSILON`.
- **Zone intersected with plot range, not clipped afterward.** Computing `Math.max(xMin, zone.min)` and `Math.min(xMax, zone.max)` *before* calling `xScale` means the rect's x-coordinates are guaranteed inside the plot frame. Skipping this would work for cases where the zone is entirely inside the plot range, but for a bull call spread with the upper strike near the edge of the ±30% window (`xMax ≈ upperStrike × 1.3`), the zone's right edge is the probe (`2×maxStrike + 1`), which is *way* outside the plot. Without intersection, `xScale` would extrapolate to an off-frame coordinate and the rect would extend past the chart border. The clip makes the zone visually reach "to the edge of the chart" in that case, which is the correct semantic anyway — *the zone continues past what we can show*.
- **Fill opacity layering.** Chose `0.08` for the zone rect — *below* the existing triangular-fill opacity of `0.10`. My first attempt used `0.20` thinking "highlight stronger than background," but the result was a distracting hard-edged band that competed with the actual curve. Lower opacity gives a subtle tint that reinforces rather than interrupts the existing P&L polygon. The zone rect draws *after* the polygons because layering order = rendering order in SVG; since the polygon is semi-transparent, the composited result reads as "deeper green inside the flat-band."
- **`PayoffExtremum` as a named type.** The old `{ value, bounded, atPrice }` shape was spelled out twice (once per side) in the interface, and the new field added a third common shape. Pulling out `PayoffExtremum` kept `maxProfit` and `maxLoss` as symmetric named references to the same interface, which reads better when you want to write code like `function foo(e: PayoffExtremum)`. No existing callers reference the old inline shape by name — all access is via the parent `PayoffExtrema` object — so the rename is a non-breaking refactor.
- **Single-point zone rendering gate.** Without `zone.max > zone.min`, a long-straddle loss zone (single-point V at the strike) would render as a zero-width rect. SVG's handling of zero-width rects varies by renderer — some browsers render a 1px-wide line at the x-coordinate, which would look like a stray vertical mark on the chart. Skipping the render for `max === min` avoids the undefined behavior and matches semantic intent (a single-point extremum isn't a "zone" worth highlighting).
- **No RTL component test for the zone rects** — still no jsdom in the repo. The pure-logic tests validate the zone fields in `expirationExtrema`'s output; the visual rendering is manual-inspection-only (same posture as the rest of PayoffDiagram).

### Next
Bonus polish list remaining:
1. **RTL/jsdom setup** — unchanged. Now the one remaining infra task. Would unblock component-level tests across all the visual refinements shipped over the last four iterations (annotations, slider, classifier header, analytical BE lines, zone rects).
2. **`## DONE`** — Spec's 24-item list is complete; five bonus polish items (annotations, slider, classifier reuse, analytical BE numeric + visual, strategy-aware zones) have shipped. The product is feature-complete with diminishing returns on further visual polish. Marking done on the next iteration unless a new direction emerges.

## Iteration: 2026-04-17 14:55

### Picked
The shared PaperAccount makes options fills flow into the existing PnL calendar (`src/components/pnl/PnlCalendar.tsx`) because they share the `fills[]` ledger. Inspected the rendered day-detail table and found option fills display as raw OCC symbols (`TSLA260417C00300000`) in the Asset column, and as `BUY` / `SELL` in the Side column — indistinguishable from perp fills and not telling the user whether they opened, closed, or expired. That's a user-visible gap in a feature the spec explicitly calls out ("PnL view shows both"). Picked over RTL/jsdom setup (still multi-iteration infra) and the `## DONE` marker because it closes a concrete rendering bug in an existing view with a contained, testable change.

### Did
- `src/services/options/occSymbol.ts` — new `parseOccSymbol(symbol)` + `formatContractLabel(symbol)`. Parser splits on the fixed 15-char tail regex `(\d{6})([CP])(\d{8})`, derives UTC-midnight expiration from `YYMMDD`, and returns null on malformed input (invalid month/day, missing underlying, lowercase ticker). `formatContractLabel` renders `TSLA 4/17 $300 C` — whole-dollar strike without decimals, fractional strike with trailing zeros trimmed (`$27.5` not `$27.50`). Falls back to the raw symbol on unparseable input so non-OCC strings (perp coin names, legacy data) pass through unchanged.
- `src/services/options/__tests__/occSymbol.test.ts` — 11 tests: standard call/put, fractional strike, 1-letter (F) and 5-letter (GOOGL) underlyings, sub-dollar strikes (SIRI $6), fall-through on bad input, lowercase rejection, label formatting with trailing-zero trim, label fall-through for non-OCC strings.
- `src/components/pnl/PnlCalendar.tsx`:
  - New `isOptionKind`, `optionSideLabel`, `optionSideColor` module-level helpers. Dispatch on `LedgerKind`: `option-open` → BTO/STO, `option-close` → BTC/STC, `option-expire` → EXP (neutral gray). Perps keep their original BUY/SELL rendering.
  - Row renderer now branches on `isOptionKind(fill.kind)`. For options: Asset column shows `formatContractLabel(fill.coin)` with the raw OCC as a `title` tooltip; Side column shows BTO/STO/BTC/STC/EXP; Size column shows `{qty} ×100` with a `title` tooltip clarifying "N contracts × 100 shares"; Fee column shows a literal `-` since option fills write `fee='0'` on the ledger and the engine doesn't model option commissions yet.
- `npx tsc --noEmit` clean.
- `npm test` → 515/515 green (was 504; +11 new in `occSymbol.test.ts`, 0 regressions).

### Discovered
- **OCC parser layout is fixed-tail, variable-head.** The underlying ticker can be 1–5+ letters, but the `YYMMDD` + `C/P` + 8-digit-strike suffix is always exactly 15 characters. Slicing from the right (`symbol.slice(-15)`) and using the rest as the underlying is simpler and more robust than trying to match the underlying with a character class up front. Only after slicing does the underlying-charset guard (`/^[A-Z0-9.]+$/`) kick in — this is where lowercase input ("tsla260417C00300000") gets rejected.
- **Year handling is 2-digit.** OCC symbols use `YY` not `YYYY`. For 2026 options that's fine (`26` → `2026`). Hardcoded `2000 + yy` works through `2099`; past that OCC itself needs a new convention. Noted as a known limitation; not worth building a windowed-year scheme for a paper-trading feature that won't survive a century.
- **Month/day validation is coarse.** Accepts any 1–12 month and 1–31 day — doesn't reject Feb-30 or Apr-31. That's intentional: Yahoo's chain contractSymbols are always real expirations, and `Date.UTC(2026, 1, 30)` silently rolls to Mar-2 without throwing, so any "impossible" date still produces a valid timestamp. A stricter check would cost code complexity for zero real-world benefit — the only input source is Yahoo itself.
- **`fill.fee === '0'` for option entries, and showing `$0.0000` looks like a live fee rendered at floor.** Showing a literal `-` in the fee column for options communicates "not applicable" rather than "zero." If option commissions get modeled in a future iteration (CBOE realism), the branch can flip to render the actual fee — the dispatch is already on `isOption`.
- **The Side column color convention**. Perps use green for buy / red for sell (matching the side). Options follow the same convention for BTO/STO/BTC/STC — BTO and BTC are both "buy" fills and both green, which is the raw direction regardless of whether it opens or closes a position. The letter prefix (B vs. S) encodes direction, the suffix (TO vs. TC) encodes lifecycle. EXP uses neutral gray because expiration is not a side.
- **`title` tooltip on the Asset column** preserves access to the raw OCC symbol for power users or anyone trying to cross-reference with exchange data. Non-destructive UX — the default view is the friendly label, but the original is one hover away.
- **No RTL test for the table rendering** — same posture as the rest of the project. Pure-logic tests cover the OCC parser; visual rendering is manual-inspection. If the RTL infra lift ever happens, this is a clean test target (render PnlCalendar with a synthetic option fill and assert the formatted label appears).
- **Chain data isn't needed for formatting.** `formatContractLabel` works off the OCC symbol alone — no `OptionContract` lookup required. That matters because the PnlCalendar is rendered from the ledger, which persists across sessions; the chain data for an expired contract is long gone by the time you scroll back a month. The OCC symbol is self-describing, so historical options fills render correctly without any contract cache.

### Next
Bonus polish list remaining:
1. **RTL/jsdom setup** — same as before. Unblocks component-level tests for PnlCalendar's option-fill rendering (which is the newest untested visual surface), plus everything from prior iterations.
2. **`## DONE`** — Spec's 24-item list is complete; six bonus polish items shipped. The remaining bonus items are infra (RTL setup) with diminishing returns on feature breadth.
3. **Spread grouping in the PnlCalendar detail panel** — naturally-shaped follow-on to this iteration. Four legs opened in one spread appear as four separate rows today; grouping by `spreadId` with a strategy-label row ("Opened Call Vertical — 2 legs, net debit $430") would read like the PositionsOptions view. Adds to the same file and same helper module; a clean next step if feature polish continues.


## Iteration: 2026-04-17 15:00

### Picked
Bonus polish: **spread grouping in the PnlCalendar detail panel**. Identified as the natural follow-on at the end of iteration 14:55 — option fills were rendering as individual rows (BTO/STO/BTC/STC) with no strategy-level summary, so a four-leg iron condor opened in one click became four disconnected rows. Spread grouping collapses those into a single header row with strategy label and net premium, with click-to-expand to see the legs. Picked over RTL/jsdom setup (still multi-iteration infra) and the `## DONE` marker because it closes a UX parity gap — `PositionsOptions` already groups by spread, and the calendar ledger had all the information (`spreadId` is on every `createOptionLedgerEntry`) but wasn't using it.

### Did
- `src/services/options/pnlFillGroups.ts` — new module:
  - `FillGroupItem = SpreadFillGroup | SingleFillGroup` tagged union.
  - `groupFillsForDay(fills)` partitions by `(spreadId, lifecycle kind)`. Perp fills and 1-element option groups pass through as `SingleFillGroup`; 2+ fills sharing spreadId + kind collapse into `SpreadFillGroup`. Input order preserved — the group appears at the input position of its first fill.
  - `SpreadFillGroup.netPremium` computed as `Σ (sellSign × size × price × 100)` (buy = -1, sell = +1). Matches the OrderForm's net-debit/credit convention: positive = credit received, negative = debit paid.
  - `SpreadFillGroup.realizedPnl` is summed; `SpreadFillGroup.strategy` comes from `classifyShapes` on per-fill `LegShape`s parsed out of each fill's `coin` via `parseOccSymbol`. Fallback to `N-leg spread` if any fill has an unparseable OCC symbol (defensive — in practice all engine-emitted option fills are OCC-formatted).
  - `lifecycleVerb` returns `Opened` / `Closed` / `Expired`.
- `src/services/options/__tests__/pnlFillGroups.test.ts` — 16 tests: empty input, perp-only passthrough, single-leg option passthrough, 2-leg open group with Call Vertical classification, netPremium debit/credit signs (vertical = debit, iron condor = credit), realized PnL summation, open+close same day partitioning (two groups same spreadId), mixed perp+spread input-order preservation, expired-legs group, unparseable-OCC fallback, input-order of multiple spread groups.
- `src/components/pnl/PnlCalendar.tsx`:
  - Replaced direct `selectedData.trades.map(fill => <tr>)` with `groupFillsForDay(...).map(item => ...)`. Single items render the existing row shape unchanged (full backward compat for perps and solo option fills). Spread items render a header row (`Opened Call Vertical · 2 legs · net debit $350.00`) and, when expanded, indented leg rows with arrow prefix (`↳`) and muted colors.
  - `expandedSpreads: Set<string>` state keyed by `date|spreadId|lifecycle|groupIdx` — the groupIdx disambiguates pathological cases where the same spread appears in the same day at both lifecycles (caught by the open+close-same-day test case).
  - Header row background `#1a1f2e40` to visually distinguish from individual fills; leg rows `#0d111740` (darker) to nest visually.
  - PnL column hidden (`-`) on `option-open` group headers because open lifecycle has no realized PnL by definition; shown on close/expire.
  - `premiumLabel` formats by lifecycle: opens say "net debit $X" / "net credit $Y" (matching OrderForm); closes/expires show signed net cash flow (no debit/credit label since that terminology is about basis, not realized flows).
- `npx tsc --noEmit` clean.
- `npm test` → 531/531 green (was 515; +16 new in `pnlFillGroups.test.ts`).

### Discovered
- **Partitioning by `(spreadId, lifecycle)` is semantically richer than just `spreadId`.** Initially I considered grouping purely by `spreadId`, which would collapse the open and close of the same spread into a single "mixed" row — but that loses the user-meaningful distinction between "opened today" and "closed today." The close is the moneymaking event and deserves its own row with its realized PnL. Tested explicitly: opening sp-rr and closing sp-rr in the same day produces two groups, not one.
- **`netPremium` sign convention matches OrderForm, not engine cash-delta.** The engine's `cashDelta` formula (`signedSzi × price × 100`) and the UI's net debit/credit flipping ("net credit = positive") differ by the meaning of "positive." OrderForm says "net credit $250" = user received money = positive. Engine's cashDelta convention for a buy has signedSzi > 0 (long position) so cashDelta > 0 represents... money flowing IN to the position, which is money OUT of the account. Picked the OrderForm convention because the PnL calendar is a user-facing read-out — the label has to match what users are used to seeing pre-submit.
- **Single-leg option groups bypass the spread row.** A spreadId with only one fill in the input (either because the user really did place a 1-leg option trade — the engine still assigns a spreadId in case — or because the other legs landed on a different day due to timezone boundary rounding) renders as the existing single-row path. Avoids a useless "Opened Long Call · 1 leg" header with no leg-level detail to drill into. The 1-leg case's `assetLabel` shows the formatted contract label just as the existing code did.
- **`groupIdx` in the expansion state key matters.** Without it, two spreads with the same spreadId and different lifecycles (open + close same day) would collide at `date|spreadId|lifecycle` (fine) — but the real subtlety is that a re-render with reordered input (e.g. sorted by price instead of time) could shift groupIdx, silently invalidating the expand-state for all groups downstream. Since the ledger is append-only and the calendar doesn't re-sort, this doesn't bite in practice, but including groupIdx makes the key robust against future sort changes.
- **Rendering indented legs inside a `<tbody>` via React.Fragment** is the cleanest path — alternatives (nested table, collapsible `<details>`, separate tables) all either break the aligned column widths or require restructuring the whole table. Fragment keeps the rows in the same tbody so column widths stay consistent across header and legs. The spreadId-keyed Fragment is necessary because React needs a stable key on the outermost element returned per iteration.
- **`background: '#1a1f2e40'`** (25% alpha on the existing border color) is subtle enough not to clash with the day-cell selection color (which tints the whole panel border) but distinct enough to telegraph "this row behaves differently" on hover. The `:hover` effect from `.fill-row` (defined globally) still applies on top.
- **Strategy classification is cheap enough to run in render.** `classifyShapes` is O(n²) in leg count for the bucket loop, but max n=4, so ~16 comparisons per group. With typical daily volume (< 50 fills), total is well under 1ms per render. Didn't add `useMemo` because the grouping function already runs cold in render anyway; caching the group result would require an `fills`-identity dependency that's already stable through the paper engine's immutable append-only ledger.
- **No RTL/jsdom still**, so the spread-row rendering and expand/collapse state have no component-level tests. The pure-logic tests cover 100% of the grouping/classification/netPremium logic. This is consistent with the project's testing posture — the UI shell is a thin rendering layer over well-tested state machines.

### Next
Bonus polish list remaining:
1. **RTL/jsdom setup** — still the one remaining infrastructure piece. Would unblock component-level tests for the PnlCalendar group header rendering, expand/collapse interaction, and every prior untested UI surface (OrderForm slider, cap banner, cell-click interactions, payoff diagram annotations).
2. **`## DONE`** — Spec's 24-item list plus seven bonus polish items (annotations, slider, classifier reuse, analytical BE numeric + visual, strategy-aware zones, spread grouping) are all shipped. The feature set is complete; remaining work is only infra (RTL) and speculative polish (e.g. hover-to-preview on day cells, spread P&L over time). A reasonable iteration next to mark the spec complete and stop.

## Iteration: 2026-04-17 15:03

### Picked
The last remaining bonus item: **RTL/jsdom setup**. Every prior iteration has deferred it as "multi-iteration infra," but a closer look showed it fits in one pass — vitest already supports per-file environment pragmas, so the config change is tiny and no existing node-environment tests need to migrate. Picked over the `## DONE` marker because a concrete demo of the setup working (a real RTL test for an existing untested visual surface) is more valuable than declaring done without ever exercising the infra. NetSummary's breakeven row was the cleanest first target: pure props in → rendered text out, no store/context dependencies, and the breakeven logic was explicitly called out in two prior iterations as untested at the component level.

### Did
- `package.json` — added devDeps `jsdom@29`, `@testing-library/react@16`, `@testing-library/jest-dom@6`, `@testing-library/user-event@14`. React 18 + RTL 16 is compatible (RTL 16 supports React 18+).
- `vitest.config.ts` — added `plugins: [react()]` so `.tsx` imports transform correctly under vitest; extended `include` to match `*.test.{ts,tsx}`; wired `setupFiles: ['src/test/setup.ts']`. Kept `environment: 'node'` as the default (535+ existing node tests stay in node), and opted specific `.tsx` files into jsdom via a per-file `// @vitest-environment jsdom` pragma. Rejected the deprecated `environmentMatchGlobs` approach after vitest logged a deprecation warning on first run.
- `src/test/setup.ts` — imports `@testing-library/jest-dom/vitest` (adds `toBeInTheDocument` etc. matchers) and registers `afterEach(cleanup)` so DOM nodes unmount between tests. Safe to load globally: `cleanup()` is a no-op when no RTL roots are mounted, and the jest-dom matchers only fire when an assertion uses them.
- `src/components/options/__tests__/NetSummary.test.tsx` — first RTL test file:
  - 6 tests covering the breakeven-row formatting that's been in production since iteration 14:45 without a component-level assertion: empty-state prompt, "none" for a fully-hedged flat payoff (long call + short same call), single BE for a long call (K + premium = $102), two BEs with em-dash for a long straddle ($95 – $105), net-debit vs. net-credit label dispatch, all five greek labels present (Delta/Gamma/Vega/Theta/Rho).
  - Uses small factory helpers (`contract(partial)`, `leg(side, c, qty)`) that mirror the style of the existing pure-logic component tests (ChainGrid.test.ts).
  - Assertions use `screen.getByText()` for text content and `container.textContent` regex for label fragments that overlap with unrelated text (e.g. "Net Debit" vs. "Per share").
- `npx tsc --noEmit` clean.
- `npm test` → 537/537 green (was 531; +6 new, 0 regressions).

### Discovered
- **Pragma vs. config-level environment selection.** Vitest supports three ways to set the test environment: (1) global `test.environment` in config, (2) the now-deprecated `environmentMatchGlobs`, and (3) a per-file `// @vitest-environment jsdom` docblock. Initially set up the config with `environmentMatchGlobs: [['src/**/*.test.tsx', 'jsdom'], ['src/**/*.test.ts', 'node']]`, which worked but printed a deprecation warning. The vitest 3.x replacement is `test.projects`, but that's meant for fundamentally different test suites, not per-file environment split. The pragma approach is the intended path — simpler, zero config churn, the intent sits next to the test that needs it. Rewrote after the first test run.
- **`cleanup()` is safe to register globally even when some files run in node.** Registering `afterEach(cleanup)` in a setupFile runs it for every test, including pure-logic tests that don't use RTL. The function iterates over an internal `mountedContainers` set that's empty unless `render()` was called, so node-environment tests see a no-op. No guard needed. Verified by the full 537-test run.
- **The `react()` plugin is needed in `vitest.config.ts` even though it's already in `vite.config.ts`.** Vitest uses its own config root when `vitest.config.ts` exists — it doesn't merge in `vite.config.ts` plugins. Without the plugin, `.tsx` imports fail with "Unexpected token <" because vitest's esbuild loader doesn't handle JSX by default. Easy to miss; the error message points at the test file, not the missing plugin.
- **React 19 vs. React 18 RTL pairing.** The repo is on React 18.3. Recent RTL (15+) dropped React 17 support but supports React 18 and 19. RTL 16.x is the current major; `@testing-library/react@16` installs cleanly against React 18.3 (peer-dep check satisfied because RTL 16 lists `react@^18 || ^19`). Didn't pin to older RTL; the project has no reason to stay on a pre-18-compatible version.
- **`@testing-library/jest-dom/vitest` is the correct import path**, not `/extend-expect` (which is Jest-specific). The `/vitest` entry registers matchers onto vitest's `expect` global. Got this right on the first read of the jest-dom README; noting it because the `/extend-expect` path is what shows up in old Jest tutorials and would silently no-op with vitest (the matchers would be accessible via `.not.toBeInTheDocument()` but produce wrong error messages).
- **`getByText('Net Debit')` fails on fragmented text nodes.** The NetSummary header renders "Net Debit" as a single text node inside a `<span>`, but the adjacent `<span>` with the dollar amount is a sibling. `getByText('Net Debit')` on the text-only version works, but if the same span held mixed children (`<span>Net <b>Debit</b></span>`) it would fail. For the direction-label test I used `container.textContent.toMatch(/Net Debit/)` to sidestep the fragment risk — more robust against future markup tweaks that nest inline formatting.
- **Test count bookkeeping.** Prior LEARNINGS entries have been meticulous about tracking the test count delta (was 531, now 537). This iteration adds the first jsdom-environment tests, which cost ~860ms of extra setup time for the jsdom VM warmup. The total suite runtime went from ~1.3s to ~1.58s — noticeable but not alarming. Scaling further (dozens of `.tsx` tests) may warrant reconsideration.
- **What's now testable that wasn't before.** The PayoffDiagram's SVG output, the OrderForm's leg list + cap banner + qty stepper, the ChainGrid cell-click interactions (via user-event), the PositionsOptions expand/collapse, the PnlCalendar spread-group expansion — all these are now reachable with unit-grade component tests. The groundwork is in place; each future iteration can pick the next surface to cover without re-doing infra.

### Next
Bonus polish list remaining:
1. **Broaden RTL coverage** — six jsdom tests is just a proof of concept. The highest-value next surfaces are:
   - OrderForm cap banner + qty stepper + submit-disabled states.
   - ChainGrid cell-click → leg-toggle interaction (the core user action of the whole feature, currently only verified manually).
   - PositionsOptions spread expand/collapse.
...
## Iteration: 2026-04-17 15:15

### Picked
Third RTL coverage surface from iteration 15:10's "Next": **PositionsOptions RTL tests**. The prior iteration explicitly flagged this as the next pick — close-button dispatch is a money-moving action that was still manual-only, and the component has several behaviour branches (chain-matches / market-open / leg count) that gate a disabled button with a stateful tooltip. Picked over PayoffDiagram SVG tests (more fiddly, lower regression risk) and the `## DONE` marker (premature while the prior iteration explicitly called for this surface).

### Did
- `src/components/options/__tests__/PositionsOptions.test.tsx` — 14 jsdom tests covering the full component surface:
  - **Empty state**: no positions → "No open option positions." and no header chip rendered.
  - **Spread header**: strategy label, leg count ("1 leg" / "2 legs"), section header with spread count `(N)`.
  - **Entry label dispatch**: long leg → "Debit $200.00" (red); short leg → "Credit $300.00" (green). Mark/PnL show em-dash when chain is absent.
  - **Expand/collapse**: click spread row → `LONG 1` / `SHORT 1` badges appear; second click collapses them back. "Load T's chain above" hint shows only when chain doesn't match.
  - **Close-button gating**: disabled + "Market closed" tooltip when `marketOpen=false`; disabled + "Load AAPL's chain to enable closing" when loaded chain underlying ≠ leg underlying; enabled + "Close all legs at current mid" tooltip when chain matches.
  - **Close-button dispatch**: click invokes `engine.closeOptionSpread(spreadId, [...calls, ...puts], { fillModel: 'mid' })` with exact payload.
  - **Feedback banners**: success → green `Closed 1 leg · PnL +$150.00`; error → red verbatim engine error string.
  - **Sort order**: multiple spreads rendered earliest-opened first regardless of `paperOptionPositions[]` order.
  - **Live mark + PnL**: chain matches → Mark shows `$310.00` (entry = 2, mid = 3.10 → 1 × 3.10 × 100 = 310), PnL shows `+$110.00` signed green.
  - **Net Greeks in expanded body**: all five labels (Δ Γ ν Θ ρ) present only when chain matches.
- `fakeEngine()` helper returns a stub exposing only `closeOptionSpread` (the one method the component calls). Cast through `unknown` to `PaperEngine` to satisfy the prop type without needing a full engine instance — the component never touches any other method.
- `legJSON()` factory uses a narrow `LegJSONInput` interface (not `Partial<OptionPositionJSON>`) so the caller can pass `strike: number` / `szi: number` / `entryPx: number` instead of strings for readability. Internally coerces via `toString()`.
- Store state reset via `useAccountStore.setState({ paperOptionPositions: [] })` in `beforeEach`; per-test positions set with `useAccountStore.setState({ paperOptionPositions: [...] })` before `render`.
- `npx tsc --noEmit` clean.
- `npm test` → 572/572 green (was 558; +14 new, 0 regressions).

### Discovered
- **Zustand `setState` in beforeEach is sufficient** — no `act()` wrapping needed because the setState fires *before* the next test's `render()`, so there's no mounted component subscribed to the store at the moment of mutation. The store's internal subscribers Set is empty between the prior test's `cleanup()` and the next test's `render()`. Initial concern about act warnings turned out to be noise from the component's *own* state updates during `fireEvent.click`, which `fireEvent` already wraps in `act` — harmless warnings, tests all pass, dropped the extra `act()` helper and kept the code simpler.
- **Factory-object type for `legJSON`**: `Partial<OptionPositionJSON>` intersected with required fields looks clean but forces strike/szi/entryPx to be strings at the call site, which drowns out the actual test data in quote marks. Declaring a separate `LegJSONInput` interface that allows `number | string` for those fields (and is `number` for most call sites) made test data read naturally: `szi: 1`, `entryPx: 2`, `strike: 100`. Small ergonomic win per test — might promote to a shared test-utils module if a fifth jsdom test file needs it.
- **`contract()` factory needed explicit `type` and `strike` in the return even though they're spread from `partial`**. TypeScript narrows `Partial<OptionContract>` at the property level, so `...partial` of a `Partial` can't satisfy required `type`/`strike` fields — had to spread and then explicitly set `type: partial.type, strike: partial.strike` at the top. Matches the pattern used in OrderForm.test.tsx and ChainGrid.test.tsx; copy-paste carried forward cleanly.
- **Finding the spread header row to click for expand/collapse**: the header is a plain `<div>` with no role. Picked `screen.getByText(/Long Call\s*·\s*1 leg/)` — a textContent regex that matches the strategy-separator dot — because it's semantically stable across classifier renamings (just needs the dot). Same technique that caught on in OrderForm's strategy-label test last iteration.
- **The "Close" button's disabled state has three distinct tooltip messages**. Three separate tests assert each branch because the tooltip selection logic in `SpreadRow` is a 3-level ternary (`!marketOpen ? 'Market closed' : !chainMatches ? 'Load X' : legsPriced<total ? 'missing quotes' : 'Close all legs'`). Skipped the "missing quotes" branch — would require constructing a chain with partial-side coverage, and the logic is the same ternary slot as the other branches. Covered 3 of 4 paths; "missing quotes" regression would still surface from the enabled+click test failing.
- **`fireEvent.click` on the spread header row**: the row uses `onClick={onToggle}` at the outermost div level, and the Close button inside that row does `e.stopPropagation()`. Clicking the header text (which is a deeply-nested span inside the row div) bubbles up through the span chain to the div's onClick — works correctly; the nested-click concern only matters if a child stops propagation, which only the Close button does. The expand/collapse toggle tests are clean because they target the strategy-label span, not the Close button.
- **`within(container).queryByText(...)` vs. global `screen.queryByText(...)`**: used `within(container)` in the Net Greeks test because after several `fireEvent.click`s the DOM has nested renders and a global `screen` query finds the text in the OUTSIDE section (the "Load X's chain" hint is specifically *inside* the expanded body, which is the `container`'s scope). Same practical concern as the ChainGrid tests but first time it mattered here — keeping the `within(...)` habit means jsdom tests in this repo stay robust against unrelated DOM changes.
- **Test count audit**: was 558 → now 572 (+14). Suite runtime went from ~2.40s to ~2.81s (jsdom share now 41/572 ≈ 7.2%, PositionsOptions was 493ms total — heaviest jsdom file because it renders 2-3 spread rows in several tests and each row renders the spread summary helpers). Still well under 3s. If the PayoffDiagram and PnlCalendar tests land too, jsdom runtime could approach 3s — that would be the point to evaluate whether to split the jsdom tests into a separate vitest project for parallelism.
- **What's still not tested at the component level**: PayoffDiagram SVG elements (BE lines, zone rects, extrema annotations), PnlCalendar spread-group header + expand/collapse, SymbolSearch debounced dropdown. Each is a half-to-full iteration. PositionsOptions was the money-moving-action risk; remaining surfaces are visual/interactive polish.

### Next
1. **PayoffDiagram RTL tests** — next on the list from iteration 15:03. Verify the component renders (a) breakeven vertical lines (count matches `analyticalBreakevens(legs)` filtered to plot range), (b) max-profit/max-loss horizontal lines when bounded, (c) zone rects for strategies with flat extremum bands (iron condor, vertical), (d) "Unlimited loss" callout for naked short calls. SVG-path-heavy so assertions will count elements + check key attributes rather than full-DOM snapshots. ~Full iteration.
2. **PnlCalendar spread-group RTL tests** — verify grouped option fills render with strategy label + net-premium, click-to-expand reveals per-leg rows, solo fills keep their single-row layout. Lower risk than the payoff tests (simpler DOM) but touches the PnL calendar which has perp+option mixed rendering. ~Half iteration.
3. **`## DONE`** — Spec's 24-item list + nine bonus items shipped; RTL coverage now has three jsdom test files spanning NetSummary, OrderForm, ChainGrid, and PositionsOptions — the core interactive surfaces are covered. Remaining untested surfaces (PayoffDiagram SVG detail, PnlCalendar spread-group, SymbolSearch) are visual-polish territory. A reasonable point to mark done if the coverage-broadening push stops.


## Iteration: 2026-04-17 15:22

### Picked
Fourth RTL coverage surface — **PayoffDiagram SVG RTL tests**. Iteration 15:15's "Next" put it first in priority: the component renders break-even lines, max-profit/max-loss zone rects + annotations, and unbounded-risk callouts that had no component-level coverage and zero regression protection beyond manual visual inspection. Picked over PnlCalendar spread-group tests (simpler DOM, lower risk, covered by `pnlFillGroups.test.ts` for grouping logic) and `## DONE` (premature while the prior iteration's explicit next-pick is still open).

### Did
- `src/components/options/__tests__/PayoffDiagram.test.tsx` — 23 jsdom tests organized into six describe blocks:
  - **Guard paths** (2): null render for empty legs; SVG width/height/viewBox wired through to props.
  - **Long call** (3): exactly one BE label at K+premium ("BE 102"); "Unlimited profit" callout; bounded max-loss annotation "Max -$200".
  - **Short call** (1): "Unlimited loss" callout present (and "Unlimited profit" absent).
  - **Long straddle** (1): two BE labels at $95.00 / $105 (fmtPrice uses 2dp for < 100, integer for >= 100).
  - **Bull call spread** (4): single BE at "BE 102"; both "Max +$800" and "Max -$200" annotations; exactly 2 zone rects (profit band S >= 110, loss band S <= 100); no "Unlimited" callout on a fully bounded spread.
  - **Iron condor** (4): two BE labels; two zone rects (bounded both sides); "Max +$100" / "Max -$400" annotations; no "Unlimited" callout.
  - **Structural elements** (8): two `<path>` elements (one with stroke-dasharray "3 3" for Today, one solid for Exp); two `<polygon>` fill bands (green above-zero + red below-zero); x-axis labels for xMin/spot/xMax; zero + spot + BE lines distinguishable by stroke-dasharray ("2 3" spot, "1 2" BE); "Exp"/"Today" legend; y-axis zero label; "$Xk" thousands formatting; structural element-count canary for a long call (2 rects: frame + loss zone).
- `nowSec={NOW_SEC}` passed to every render so the BS-today curve is deterministic (doesn't depend on `Date.now()` at test time).
- `contract(partial)` and `leg(side, c, qty)` factories copied from ChainGrid.test.tsx verbatim — same pattern continues to read cleanly.
- `npx tsc --noEmit` clean.
- `npm test` → 595/595 green (was 572; +23 new, 0 regressions).

### Discovered
- **`fmtPrice` has an asymmetric formatting rule.** Values >= 100 render as integer via `toFixed(0)`; values < 100 render with 2 decimals via `toFixed(2)`. So a long-straddle centered at K=100 with BEs at 95 and 105 produces labels `['BE 95.00', 'BE 105']`. Initial draft of the straddle test expected symmetric labels `['BE 95', 'BE 105']` and failed. Kept the exact formatter output rather than asserting a regex, because the asymmetry is itself a UI contract — a future refactor that changes the < 100 branch to integer formatting would be a visible shift in how cheap-stock BE labels render, and catching it here is good. Same pattern applies to x-axis labels for underlyingPrice=100: xMin=70 renders as "70.00", xMax=130 as "130".
- **A long call renders TWO `<rect>` elements, not one.** I initially expected only the plot-frame rect because "long call has unbounded profit, so no max-profit zone." But the max-LOSS side is bounded: `-premium × 100 = -$200` holds over the entire OTM flat band [S=0, S=strike]. That's a real zone, so the component renders a red loss-zone rect over it. The bull call spread similarly has TWO zone rects (profit band above upper strike + loss band below lower strike) — not just the max-profit rect as I'd guessed. Takeaway for future payoff tests: the zone-rect count is `bounded-profit-sides + bounded-loss-sides`, each zone is rendered whenever its `zone.max > zone.min` AND falls within the plotted x-range. Iron condor (2 rects) and bull call spread (2 rects) happen to match, not because of any "two for a bounded spread" rule.
- **`stroke-dasharray` as a dimension for counting lines.** The component uses three distinct dash patterns: zero line is solid (no attribute), spot line is "2 3", BE lines are "1 2", max-profit line is "4 3". Asserting by dash pattern is cleaner than counting all `<line>` elements because the exact count shifts with strategy (0 BE for a hedged spread, 2 BE for a straddle, 4+ BE for a four-leg with multiple crossings). Kept the canary assertions shape-based (`dashes.has('2 3')`) not count-based.
- **The `Max …` annotation text uses `startsWith('Max ')` as the filter.** There's a subtle overlap risk: `fmtMoney` emits `$X` or `$Xk`, so a "Max +$800" annotation is cleanly distinct from a BE label ("BE 102"). But if future copy changes to e.g. "Max profit: +$800", the startsWith filter would still match. Kept the loose filter because the exact string `'Max +$800'` is asserted via `toContain(...)` in the same tests — the filter just narrows the candidate set.
- **`fmtMoney` thousands uses lowercase "k" with one-decimal precision.** For the $Xk formatting test I used a 10-contract long call (bid=ask=20) to force values well over $1000: expected y-axis labels like "$20k" / "-$20k". jsdom-rendered text matched the regex `/\$\d+(\.\d)?k/`. Didn't hardcode exact values because the yMargin (8%) slightly inflates yMin/yMax before formatting, and the exact figure is incidental to the formatting rule.
- **Test count audit**: was 572 → now 595 (+23). jsdom share now 64/595 ≈ 10.8%; suite runtime went from ~2.81s to ~4.12s. PayoffDiagram is the heaviest jsdom file at ~405ms (23 renders × ~18ms per render; each render executes the `useMemo` pipeline for buildPayoffCurve over 121 samples × 2 curves). Combined with OrderForm (1576ms) and ChainGrid (978ms), jsdom now dominates suite runtime. If the PnlCalendar tests land too, total runtime will approach 5s — that's the point where splitting jsdom into a separate vitest project (parallel workers) becomes worth the config complexity. Not concerning yet.
- **Counting SVG elements via `container.querySelectorAll('tag')` works cleanly in jsdom** — no need to scope with a `<svg>` namespace or `getElementsByTagNameNS`. The `<svg>` elements are in the XHTML namespace in jsdom's DOM, and `querySelectorAll('rect')` / `querySelectorAll('path')` match by local name. Confirmed by inspection across all 23 assertions.

### Next
Bonus polish list remaining:
1. **PnlCalendar spread-group RTL tests** — render grouped option fills with strategy label + net-premium row, click-to-expand reveals per-leg rows, solo fills keep their single-row layout. Lower risk than PayoffDiagram (simpler DOM) but touches the PnL calendar which has perp+option mixed rendering. ~Half iteration.
2. **SymbolSearch debounced dropdown + recents** — a smaller surface: on-change debounce to 300ms, dropdown items render from prefix match, clicking a hit calls `onSelect`. The one piece of this component that's visual-only and likely to regress silently is the recents list wiring to `localStorage`. ~Half iteration.
3. **`## DONE`** — Spec's 24-item list plus nine bonus items plus now-five jsdom test files covering NetSummary, OrderForm, ChainGrid, PositionsOptions, PayoffDiagram. The money-moving-action and visual-chart surfaces are both covered. PnlCalendar and SymbolSearch are visual/interactive polish with pure-logic test coverage at their respective underlying layers. Reasonable point to mark done if the coverage push stops.


## Iteration: 2026-04-17 15:25

### Picked
Fifth RTL coverage surface from iteration 15:22's "Next": **PnlCalendar spread-group RTL tests**. Iteration 15:22 put PnlCalendar first in priority order (before SymbolSearch and `## DONE`). The calendar has perp + option mixed rendering, a 3-lifecycle (open/close/expire) grouping flow, and an independent per-group expand/collapse state machine — all verifiable only by manual inspection prior to this iteration. Picked over SymbolSearch because the calendar surface touches money-visible PnL display and has more interaction branches; picked over `## DONE` because one more RTL surface lands the whole grouping-logic-presentation pipeline under component coverage.

### Did
- `src/components/pnl/__tests__/PnlCalendar.test.tsx` — 12 jsdom tests organized into one describe block:
  - **Solo rows (2)**: a perp fill renders as single BUY row (no `▸`/`▾` caret); a single-leg option-open fills renders as single BTO row (no caret), asset column uses `formatContractLabel` ("TSLA 4/17 $380 C").
  - **Spread header labeling (2)**: 2-leg debit vertical renders "Opened … 2 legs · net debit $400.00"; 2-leg credit vertical renders "net credit $300.00". Strategy label asserted only via regex `/Opened/` and `/2 legs/` — exact strategy wording (Bull Call vs. Call Vertical) is owned by `strategy.test.ts`, not this test.
  - **Expand/collapse (1)**: click header → BTO/STO leg badges + `$6.00`/`$2.00` price cells appear, caret flips `▸→▾`; click again → leg rows hide, caret flips `▾→▸`.
  - **Iron condor close (1)**: 4 `option-close` fills of the same spreadId → one header "Closed … 4 legs · $145.00" (PnL = 50+25+30+40). Lifecycle `option-close` uses "net $X" wording (no debit/credit label — asserted with `.not.toMatch(/net debit|net credit/)`). Expand → 2 BTC + 2 STC badges.
  - **Expire lifecycle (1)**: 2 `option-expire` fills → one "Expired" header; expand → 2 EXP badges.
  - **Grouping boundaries (2)**: spreads with distinct `spreadId`s on the same day render as 2 separate headers (2 `getAllByTitle(/expand legs/)` matches); same `spreadId` with different lifecycles (open + close on the same day) render as 2 separate groups.
  - **Per-group expand state (1)**: expanding group A leaves group B collapsed. Asserted via `getAllByTitle(/collapse legs/)` returns 1 + `getAllByTitle(/expand legs/)` returns 1, plus group A's prices visible and group B's prices hidden.
  - **Mixed day (1)**: perp fill + 2-leg option spread on the same day → perp renders as BTC/single row, spread renders as one Opened header (so `getAllByTitle(/expand legs/)` returns exactly 1).
  - **Em-dash PnL for opens (1)**: opened-spread header row's PnL-column cell is literally `-` (not `$0.00`) because `showPnl = lifecycle !== 'option-open'`.
- `seedFills(fills)` helper mutates the active `PaperAccount`'s `fills` via `usePaperAccountsStore.setState({ accounts: [...] })` — same pattern as PositionsOptions.test.tsx's store-slice override.
- `tsInCurrentMonth(hour, minute)` picks a timestamp on day 15 of the *current* month at test time, since `PnlCalendar` defaults to the current month on mount and I didn't want to drive the `←`/`→` month-nav buttons per test.
- `clickDayCell(dateStr)` helper finds the seeded day by scanning `getAllByText(dayNum)` for the parent cell whose textContent contains "trade" (the fills-present cells render "N trades" text).
- `beforeEach` resets `fills: []` on the active account so tests don't leak state across iterations.
- `npx tsc --noEmit` clean.
- `npm test` → 607/607 green (was 595; +12 new, 0 regressions).

### Discovered
- **jsdom serializes inline styles as kebab-case.** My first `clickDayCell` draft used `el.closest('[style*="minHeight"]')` — never matched because jsdom's serialized `style` attribute reads `padding: 8px 6px; min-height: 70px; background: rgba(14, 203, 129, 0.094); ...`. The React JSX `style={{ minHeight: 70 }}` gets normalized to `min-height` in the serialized DOM. Takeaway for future PnlCalendar-style tests: don't rely on attribute-contains selectors with camelCase style properties. Either use kebab-case in the selector (`[style*="min-height"]`), or — better — navigate by DOM structure (parentElement / `textContent` match) which is what the final helper does. All 12 tests failed at the helper on the first run and went green after switching to `el.parentElement` + textContent filter.
- **`usePaperAccountsStore.getState()` + `.setState()` is the right seam for seeding `fills`.** The `PnlCalendar` component calls `getActiveAccount()` via a selector; to override its `fills` I map over `accounts` and patch the active one. Cleaner than mocking the whole store hook with `vi.mock`, and survives the React subscription path unchanged. Same pattern as `useAccountStore.setState({ paperOptionPositions: [...] })` in PositionsOptions.test.tsx but for the other store (the perp-focused one).
- **Date-of-day ambiguity in calendar grids**: there's usually exactly one "15" in the current month's grid, but if the previous month's 15th spilled into a trailing row of the prior month, `getAllByText('15')` could return 2+ matches. The helper scans all of them and picks the one whose parent cell's textContent contains "trade" (which only the seeded in-month day has). Robust enough; in practice during testing only 1 match appeared because day 15 is mid-month and never spills.
- **`groupFillsForDay`'s lifecycle split is visible through the UI**: opening an iron condor and closing it on the same day renders as TWO separate spread headers (one "Opened", one "Closed"), not one. This matches the grouping logic in `pnlFillGroups.ts`: the bucket key is `${spreadId}|${kind}` so open-buy vs. close-sell fills of the same `spreadId` go to different buckets. Good invariant — users see a clear "open" event and a separate "close" event with its own realized PnL. Added a test to pin this to avoid future "let's merge same-spread-same-day fills" churn.
- **"net debit / net credit" vs. "net $X" wording by lifecycle**: `premiumLabel()` (in `PnlCalendar.tsx:34`) shows `credit`/`debit` only for `option-open` fills and just signed-dollars for `option-close`/`option-expire`. This is a deliberate UI choice — post-entry cash flows aren't really "debits" or "credits" in trader parlance, they're settlement inflows/outflows. Captured with a negative assertion (`.not.toMatch(/net debit|net credit/)`) on the iron-condor close test so a future refactor that e.g. unifies the wording would fail the test and prompt a conversation rather than silently shipping.
- **The em-dash PnL display depends on `showPnl = lifecycle !== 'option-open'`**: opens show `-`, closes/expires show `$X.XX`. The test uses `within(headerRow).getAllByRole('cell')` and inspects the last cell — textContent `-` (not `$0.00`) — to pin the intended UX.
- **Test count audit**: was 595 → now 607 (+12). Suite runtime went from ~4.12s to ~5.79s wall-time (vitest total including transform/setup); the jsdom file share is now 76/607 ≈ 12.5%, and PnlCalendar adds ~2.1s to suite time (12 tests × ~180ms per render — heavier than NetSummary because each PnlCalendar render mounts a 42-cell grid + a 5-column table and re-runs `groupFillsByDay` over the fixtures). Well under the 10s mental-model threshold where "tests are slow" starts to become a friction point. If the next iteration lands SymbolSearch's dropdown tests too, jsdom share would be around 14% and suite time around 6.5s — still comfortable.
- **Fresh account state per test via `beforeEach`**: the default account from `loadAccounts()` persists across tests (localStorage is in-memory in jsdom but the store is a singleton). Reset `fills: []` each test explicitly. Works cleanly with the existing `afterEach(cleanup)` — the store survives test boundaries, but its `fills` get nuked to a known empty state.

### Next
Bonus polish list remaining:
1. **SymbolSearch debounced dropdown + recents** — the only unruly untested component-level surface left. Its logic has (a) a 300ms debounced on-change that filters a static popular list, (b) recents persisted to `localStorage`, (c) free-form-entry fallback when no match. Tests would mount the component with a stub `onSelect`, advance vitest's fake timers by 300ms (`vi.useFakeTimers()` + `vi.advanceTimersByTime(300)`), and assert the dropdown DOM. ~Half iteration.
2. **`## DONE`** — Spec's 24-item list plus nine bonus items. All six jsdom test files now cover NetSummary, OrderForm, ChainGrid, PositionsOptions, PayoffDiagram, and PnlCalendar. The money-moving-action (close-spread dispatch), visual-chart (payoff curves + BE lines), and historical-view (calendar + spread groups) surfaces are all component-tested. Remaining untested surface (SymbolSearch) is entry-point polish with the underlying `symbols.ts` helper already unit-tested. A fair point to mark the spec complete if the coverage push stops.


## Iteration: 2026-04-17 15:30

### Picked
Sixth RTL coverage surface from iteration 15:25's "Next": **SymbolSearch debounced dropdown + recents**. This was the last untested component-level surface — iteration 15:25 explicitly flagged it as the next pick, with the alternative being a `## DONE` marker. Picked the test over the `## DONE` because the component has three interaction branches (debounce, keyboard nav, free-form fallback) plus a localStorage persistence path, and landing its coverage rounds out the jsdom test files to seven — every interactive component has RTL tests.

### Did
- `src/components/options/__tests__/SymbolSearch.test.tsx` — 15 jsdom tests organized into one describe block:
  - **Placeholder rendering** (2): `value=null` → "Search symbol (e.g. TSLA, SPY)"; `value="TSLA"` → placeholder is "TSLA".
  - **Closed dropdown** (1): before focus, popular-hit rows and the "Free-form" label are absent.
  - **Debounce timing** (2): 300ms fake-timer advance. At t=299 TSLA hit absent; at t=300 TSLA + "Tesla Inc." both present. Also the typing-restart path: a 2nd change at t=200 cancels the first timer, new debounce fires at t=500.
  - **Commit paths** (3): (a) mouse-down on a hit row → `onChange('TSLA')` with upper-case normalization from "tsla" lowercase input; (b) Enter key commits the active row; (c) free-form Enter on a no-hits query commits the typed uppercase query.
  - **Free-form row** (1): query "ZZZZ" with no hits shows a ZZZZ row with "Free-form" secondary label.
  - **Escape** (1): key closes the dropdown without invoking onChange.
  - **Keyboard nav** (1): ArrowDown moves the active (highlighted background) row from index 0 to 1. Row discovery via `container.querySelectorAll('div[style*="cursor: pointer"]')` — picks the interactive hit-row divs by inline style.
  - **Recents** (2): (a) seeded localStorage → 2 recent rows render under a "Recent" header pill on focus with empty query; (b) a mouse-down commit persists the committed symbol to `hl-options-recent-symbols`.
  - **State cleanup** (1): input.value is empty after commit.
  - **Disabled prop** (1): input has `disabled`; changing value + advancing timers doesn't open the dropdown (no "Tesla Inc." rendered).
- `vi.useFakeTimers()` + `vi.advanceTimersByTime(ms)` wrapped in `act(() => ...)` via an `advance(ms)` helper. Wrapping the timer advance in `act` is required because the debounce's `setTimeout` callback calls `setDebounced(...)` which triggers a React state update — without `act`, the re-render isn't flushed before the next assertion.
- `vi.useRealTimers()` in `afterEach` so the test teardown (library cleanup + other files) doesn't leak fake timers into later test files.
- `window.localStorage.clear()` in `beforeEach` to reset the recents list between tests.
- `npx tsc --noEmit` clean.
- `npm test` → 622/622 green (was 607; +15 new, 0 regressions).

### Discovered
- **`vi.advanceTimersByTime` must be wrapped in `act()` when the timer callback sets React state.** The debounce's `setTimeout(() => setDebounced(query.trim()), 300)` is a React-state transition, and advancing timers outside `act` produces the "An update to SymbolSearch inside a test was not wrapped in act(...)" warning AND the assertion sees stale DOM. Extracted an `advance(ms)` helper that wraps `vi.advanceTimersByTime` in `act` — one line per call site, eliminates a whole category of flake.
- **Text "Recent" appears three times per recent row + once as the header.** The dropdown renders three `Recent` text nodes in the recents branch: (1) the uppercase "RECENT" header pill at the top, (2) each row's secondary-info `<span>` shows "Recent" (fallback when `r.name` is undefined), (3) each row's right-side badge shows "Recent" (conditional on `r.recent`). With 2 recents the count is `1 + 2 × 2 = 5`. Initially asserted `length === 3` and failed with the expected-to-received 3-vs-5 mismatch; corrected the assertion and left a comment on the arithmetic so future readers don't re-derive. Structural insight: if the component ever consolidates "Recent" into a single label per row, the assertion will fail and prompt a conversation rather than silently drift.
- **"SP" query doesn't yield SPY at index 0.** POPULAR_SYMBOLS iterates in declaration order, and SPOT (Spotify at line ~45) comes before SPY (line ~159). So for the ArrowDown test I switched from "SP" (where I expected SPY first) to "TSL" (where TSLA is an exact match and sits at index 0 deterministically). Also restructured the test to query rows by *structural position* (`container.querySelectorAll('div[style*="cursor: pointer"]')`) rather than by symbol label — more robust against future re-orderings of POPULAR_SYMBOLS or debounce-yielded hit sequences.
- **`useEffect([debounced, open], () => setActiveIdx(0))` resets activeIdx deterministically** — after the 300ms debounce fires, activeIdx lands at 0 regardless of prior ArrowDown presses during typing. Tested explicitly in the ArrowDown test (pre-ArrowDown: row 0 has active bg `rgb(26, 31, 46)`; post-ArrowDown: row 1 has it).
- **Disabled-prop test needs the timer advance too**, because even though the input's `disabled` attribute blocks focus/click in a real browser, jsdom permits `fireEvent.change` on disabled inputs. The actual gate is `{open && !disabled && rows.length > 0 && ...}` — the `!disabled` term in the render guard is what we're asserting here. Advanced the fake timer after typing to exercise the full debounce path, then asserted "Tesla Inc." never renders. Captures the `!disabled` render guard as a regression test that's independent of the input's `disabled` attribute.
- **Free-form path has two triggers**: (a) `rows[activeIdx] = { symbol: q }` when no popular hit has exact symbol match → Enter picks the free-form row, calls `commit(picked.symbol)`; (b) `rows` is empty → Enter falls through to `else if (query.trim()) commit(query)`. For the "zzzqqq" test the query produces a free-form row and no popular hits, so rows[0] = the free-form entry and the commit goes through path (a). Both paths produce the same uppercase-commit behaviour from the user's perspective, so asserted the outcome (`onChange('ZZZQQQ')`) not the internal branch.
- **Test count audit**: was 607 → now 622 (+15). Suite runtime stayed at ~3.16s (PayoffDiagram still heaviest at ~300ms; SymbolSearch clocks ~139ms, among the lightest jsdom files because the DOM tree is shallow — just a wrapper div + one input + one dropdown div with up to 12 row divs). jsdom share is now 91/622 ≈ 14.6% — essentially the ceiling now that every interactive component has RTL coverage. No pressure to split into a separate vitest project.
- **Seven jsdom test files**: NetSummary, OrderForm, ChainGrid, PositionsOptions, PayoffDiagram, PnlCalendar, SymbolSearch. Every interactive option component in the repo has component-level RTL coverage. The remaining non-test components are: `OptionsPage.tsx` (layout wrapper, no logic), `LegRow.tsx` (display-only, used in OrderForm tests), `ChainRow.tsx` (display-only, covered via ChainGrid), `MarketClosedBanner.tsx` (pure render of two strings), `ExpirationTabs.tsx` (pure-logic test exists).

### Next
- **`## DONE`** — Spec's 24-item iteration list is complete. Bonus polish items shipped: symbol search, expiration tabs, chain metrics customization, payoff diagram, spread-group PnL calendar, 4-leg cap enforcement, and now seven RTL-tested components. Money-moving actions (submit order, close spread, auto-exercise settlement) are all covered by unit + integration + RTL tests; visual charts (payoff diagram) have component tests for BE lines / zone rects / extrema callouts; historical PnL calendar has component tests for spread-group rendering and per-group expand state. I'd mark this done — each remaining micro-task would be polish with diminishing marginal regression protection.

## DONE

All 24 spec iteration items shipped across 35 iterations (2026-04-17 10:53 → 15:30). Nine bonus items added on top: strategy auto-detection, expiration-auto-exercise chain-refresh loop, short-leg cash-margin gating, payoff diagram with BE/zone/extrema rendering, chain-metrics column customization, per-group expand state in the PnL calendar, 4-leg cap enforcement with inline banner, spread-group labeling in the historical calendar, and seven jsdom RTL test files covering every interactive component (NetSummary, OrderForm, ChainGrid, PositionsOptions, PayoffDiagram, PnlCalendar, SymbolSearch).

Final totals: 622 passing tests across 32 test files, clean `npx tsc --noEmit`, no regressions to perps paper trading. Options and perps coexist in the same `PaperAccount` with shared USD balance and shared fills ledger. Market-hours gate on submissions. Yahoo Finance chain proxy is a Vite dev plugin (explicitly out-of-scope for production per spec).

Known simplifications documented in-code or in the spec: Black-Scholes pricing for American-style US equity options (acceptable for paper trading), conservative 5× premium cash margin for short legs (non-CBOE), no early exercise (options resolve only at expiration), Yahoo-only adapter in v1 (adapter interface ready for Deribit/Tradier swap).
